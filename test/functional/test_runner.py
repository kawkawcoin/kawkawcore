...
`test/functional/test_framework/test_framework.py:KawkawTestFramework.main`.
...
def main():
    ...
    tmpdir = "%s/kawkaw_test_runner_%s" % (args.tmpdirprefix, datetime.datetime.now().strftime("%Y%m%d_%H%M%S"))
    os.makedirs(tmpdir)
    logging.debug("Temporary test directory at %s" % tmpdir)

    # Don't run tests on Windows by default
    if config["environment"]["EXEEXT"] == ".exe" and not args.force:
        # https://github.com/KawkawChain/Kawkaw/commit/d52802551752140cf41f0d9a225a43e84404d3e9
        # https://github.com/Bitcoin/bitcoin/pull/5677#issuecomment-136646964
        print("Tests currently disabled on Windows by default. Use --force option to enable")
        sys.exit(0)

    # Check that the build was configured with wallet, utils, and kawkawd
    enable_wallet = config["components"].getboolean("ENABLE_WALLET")
    enable_cli = config["components"].getboolean("ENABLE_UTILS")
    enable_kawkawd = config["components"].getboolean("ENABLE_KAWKAWD")
    if not (enable_wallet and enable_cli and enable_kawkawd):
        print("No functional tests to run. Wallet, utils, and kawkawd must all be enabled")
...
def run_tests(test_list, src_dir, build_dir, exeext, tmpdir, use_term_control, jobs=1, enable_coverage=False, args=None, combined_logs_len=0, failfast=False, last_loop=False):
    # Warn if kawkawd is already running (unix only)
    if args is None:
        args = []
    try:
        if subprocess.check_output(["pidof", "kawkawd"]) is not None:
            print("%sWARNING!%s There is already a kawkawd process running on this system. Tests may fail unexpectedly due to resource contention!" % (BOLD[1], BOLD[0]))
    ...
    #Set env vars
    if "KAWKAWD" not in os.environ:
        os.environ["KAWKAWD"] = build_dir + '/src/kawkawd' + exeext
        os.environ["KAWKAWCLI"] = build_dir + '/src/kawkaw-cli' + exeext
...
class RPCCoverage:
    ...
    commands per `kawkaw-cli help` (`rpc_interface.txt`).
