...
The RPM spec file provided here is for Kawkaw-Core 0.12.0 and builds on CentOS
...
reference the release tarball available from https://kawkaw.net/bin/ and
`Source1` should reference the BerkeleyDB source.

Sources 10-99 are for source files that are maintained in the
[Kawkaw git repository](https://github.com/KawkawChain/Kawkaw) but are not part of
...
Currently only `Source10` is used, for the example `kawkaw.conf` file.
...
in the kawkaw git repository. At present only an SVG version of the Kawkaw
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the kawkaw
...
needed when 0.12.1 is released, a proper fix is already in the Kawkaw git
master branch.
...
idea to build Kawkaw Core against a static version of that library compiled
...
The `kawkaw.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `kawkaw-server` package. At this point, most RPM based
...
The `kawkaw-server` package should have SELinux support. How to properly do
...
Kawkaw-Core against OpenSSL or use a patched version of Boost in the build
system.
...
[Ringing Liberty](https://www.ringingliberty.com/kawkaw/). He has been
packaging Kawkaw for Fedora at least since 2012.
...
4. I renamed the `kawkaw` package that contains the Qt GUI to `kawkaw-core` as
that appears to be how the general population refers to it, in contrast to
`kawkaw-xt` or `kawkaw-classic`. I wanted to make sure the general population
