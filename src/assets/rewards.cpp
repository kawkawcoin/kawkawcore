...
bool GenerateDistributionList(const CRewardSnapshot& p_rewardSnapshot, std::vector<OwnerAndAmount>& vecDistributionList)
{
    ...
    CAmount srcUnitDivisor = COIN;  //  Default to divisor for KAW
    ...
    if (p_rewardSnapshot.strDistributionAsset != "KAW") {
        ...
        LogPrint(BCLog::REWARDS, "%s: Distribution is KAW with divisor %d\n", __func__, srcUnitDivisor);
    }
        ...
        std::string& change_address, uint256& retTxid)
{
    ...
    //  Handle payouts using KAW differently from those using an asset
    if (p_rewardSnapshot.strDistributionAsset == "KAW") {
        // Check amount
        ...
        for (int i = start; i < (int)p_pendingPayments.size() && i < stop; i++) {
            ...
            // Parse Kawkaw address (already validated during ownership snapshot creation)
