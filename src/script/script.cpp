...
const char* GetOpName(opcodetype opcode)
{
    ...
    /** KAWKAW START */
    case OP_KAW_ASSET              : return "OP_KAW_ASSET";
    /** KAWKAW END */
...
{
    if (this->size() > 31) {
        if ((*this)[25] == OP_KAW_ASSET) { // OP_KAW_ASSET is always in the 25 index of the script if it exists
            int index = -1;
            if ((*this)[27] == KAW_N) { // Check to see if KAW starts at 27 ( this->size() < 105)
                if ((*this)[28] == KAW_E)
                    if ((*this)[29] == KAW_X)
                        index = 30;
            } else {
                if ((*this)[28] == KAW_N) // Check to see if KAW starts at 28 ( this->size() >= 105)
                    if ((*this)[29] == KAW_E)
                        if ((*this)[30] == KAW_X)
                            index = 31;
            }
            ...
            if (index > 0) {
                ...
                if ((*this)[index] == KAW_T) { // Transfer first anticipating more transfers than other assets operations
                    nType = TX_TRANSFER_ASSET;
                    return true;
                } else if ((*this)[index] == KAW_Q && this->size() > 39) {
                    nType = TX_NEW_ASSET;
                    fIsOwner = false;
                    return true;
                } else if ((*this)[index] == KAW_O) {
                    nType = TX_NEW_ASSET;
                    fIsOwner = true;
                    return true;
                } else if ((*this)[index] == KAW_N) {
...
{
    // 1 OP_KAW_ASSET followed by two OP_RESERVED + atleast 4 characters for the restricted name $ABC
    return (this->size() > 6 &&
            (*this)[0] == OP_KAW_ASSET &&
            (*this)[1] == OP_RESERVED &&
            (*this)[2] == OP_RESERVED);
...
{
    // 1 OP_KAW_ASSET followed by one OP_RESERVED
    return (this->size() > 3 &&
            (*this)[0] == OP_KAW_ASSET &&
            (*this)[1] == OP_RESERVED &&
            (*this)[2] != OP_RESERVED);
...
{
    CAmount nAmount;
    return (size() > 0 && *begin() == OP_RETURN) || (size() > 0 && *begin() == OP_KAW_ASSET) || (size() > MAX_SCRIPT_SIZE) || (GetAssetAmountFromScript(*this, nAmount) && nAmount == 0);
}
