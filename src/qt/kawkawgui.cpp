...
#include "config/kawkaw-config.h"
#endif

#include "kawkawgui.h"

#include "kawkawunits.h"
...
const std::string KawkawGUI::DEFAULT_UIPLATFORM =
#if defined(Q_OS_MAC)
        "macosx"
...
const QString KawkawGUI::DEFAULT_WALLET = "~Default";

KawkawGUI::KawkawGUI(const PlatformStyle *_platformStyle, const NetworkStyle *networkStyle, QWidget *parent) :
    QMainWindow(parent),
    enableWallet(false),
    clientModel(0),
    walletFrame(0),
...
void KawkawGUI::createActions()
{
    ...
    sendCoinsAction->setStatusTip(tr("Send coins to a kawkaw address"));
    ...
    receiveCoinsAction->setStatusTip(tr("Request payments (generates QR codes and kawkaw: URIs)"));
    ...
    transferAssetAction->setStatusTip(tr("Transfer assets to KAW addresses"));
...
//    restrictedAssetAction->setShortcut(QKeySequence(Qt::ALT + Qt::Key_9));
    ...
    signMessageAction->setStatusTip(tr("Sign messages with your kawkaw addresses to prove you own them"));
    verifyMessageAction = new QAction(platformStyle->TextColorIcon(":/icons/verify"), tr("&Verify message..."), this);
    verifyMessageAction->setStatusTip(tr("Verify messages to ensure they were signed with specified kawkaw addresses"));
    ...
    openAction->setStatusTip(tr("Open a Kawkaw: URI or payment request"));

    showHelpMessageAction = new QAction(platformStyle->TextColorIcon(":/icons/info"), tr("&Command-line options"), this);
    showHelpMessageAction->setMenuRole(QAction::NoRole);
    showHelpMessageAction->setStatusTip(tr("Show the %1 help message to get a list with possible kawkaw command-line options").arg(tr(PACKAGE_NAME)));
...
void KawkawGUI::createToolBars()
{
    if(walletFrame)
    {
        ...
        label->setPixmap(QPixmap::fromImage(QImage(":/icons/kawkawcointext")));
...
//        toolbar->addAction(votingAction);
        ...
        QVBoxLayout* kawkawLabelLayout = new QVBoxLayout(toolbarWidget);
        kawkawLabelLayout->addWidget(label);
        kawkawLabelLayout->addWidget(toolbar);
        kawkawLabelLayout->setDirection(QBoxLayout::TopToBottom);
        kawkawLabelLayout->addStretch(1);
        ...
        labelCurrentMarket->setText(tr("Kawkaw Market Price"));
        ...
        // Get the latest Kawkaw release and let the user know if they are using the latest version
        // Network request code for the header widget
...
void KawkawGUI::setClientModel(ClientModel *_clientModel)
{
...
#ifdef ENABLE_WALLET
bool KawkawGUI::addWallet(const QString& name, WalletModel *walletModel)
{
    if(!walletFrame)
        return false;
...
bool KawkawGUI::setCurrentWallet(const QString& name)
{
    if(!walletFrame)
        return false;
...
void KawkawGUI::removeAllWallets()
{
    if(!walletFrame)
        return;
    setWalletActionsEnabled(false);
...
void KawkawGUI::setWalletActionsEnabled(bool enabled)
{
...
void KawkawGUI::createTrayIcon(const NetworkStyle *networkStyle)
{
#ifndef Q_OS_MAC
...
void KawkawGUI::createTrayIconMenu()
{
#ifndef Q_OS_MAC
...
#ifndef Q_OS_MAC
void KawkawGUI::trayIconActivated(QSystemTrayIcon::ActivationReason reason)
{
...
void KawkawGUI::showDebugWindow()
{
...
void KawkawGUI::showDebugWindowActivateInformation()
{
    rpcConsole->setTabFocus(RPCConsole::TAB_INFO);
    showDebugWindow();
}

void KawkawGUI::showDebugWindowActivateConsole()
{
    rpcConsole->setTabFocus(RPCConsole::TAB_CONSOLE);
    showDebugWindow();
}

void KawkawGUI::showDebugWindowPeerList()
{
    rpcConsole->setTabFocus(RPCConsole::TAB_PEERS);
    showDebugWindow();
}

void KawkawGUI::showDebugWindowNetworkTraffic()
{
    rpcConsole->setTabFocus(RPCConsole::TAB_GRAPH);
    showDebugWindow();
}

void KawkawGUI::showWalletRepair()
{
    rpcConsole->setTabFocus(RPCConsole::TAB_REPAIR);
    showDebugWindow();
}

void KawkawGUI::showHelpMessageClicked()
{
    helpMessageDialog->show();
}

#ifdef ENABLE_WALLET
void KawkawGUI::openClicked()
{
...
void KawkawGUI::gotoOverviewPage()
{
    overviewAction->setChecked(true);
    if (walletFrame) walletFrame->gotoOverviewPage();
}

void KawkawGUI::gotoHistoryPage()
{
    historyAction->setChecked(true);
    if (walletFrame) walletFrame->gotoHistoryPage();
}

void KawkawGUI::gotoReceiveCoinsPage()
{
    receiveCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoReceiveCoinsPage();
}

void KawkawGUI::gotoSendCoinsPage(QString addr)
{
    sendCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoSendCoinsPage(addr);
}

void KawkawGUI::gotoSignMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoSignMessageTab(addr);
}

void KawkawGUI::gotoVerifyMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoVerifyMessageTab(addr);
}

/** KAWKAW START */
void KawkawGUI::gotoAssetsPage()
{
    transferAssetAction->setChecked(true);
    if (walletFrame) walletFrame->gotoAssetsPage();
};

void KawkawGUI::gotoCreateAssetsPage()
{
    createAssetAction->setChecked(true);
    if (walletFrame) walletFrame->gotoCreateAssetsPage();
};

void KawkawGUI::gotoManageAssetsPage()
{
    manageAssetAction->setChecked(true);
    if (walletFrame) walletFrame->gotoManageAssetsPage();
};

void KawkawGUI::gotoRestrictedAssetsPage()
{
...
void KawkawGUI::updateNetworkState()
{
    ...
    if (clientModel->getNetworkActive()) {
        tooltip = tr("%n active connection(s) to kawkaw network", "", count) + QString(".<br>") + tr("Click to disable network activity.");
    } else {
...
void KawkawGUI::setNumConnections(int count)
{
    updateNetworkState();
}

void KawkawGUI::setNetworkActive(bool networkActive)
{
    updateNetworkState();
}

void KawkawGUI::updateHeadersSyncProgressLabel()
{
...
void KawkawGUI::setNumBlocks(int count, const QDateTime& blockDate, double nVerificationProgress, bool header)
{
    if (modalOverlay)
    {
        if (header)
...
void KawkawGUI::message(const QString &title, const QString &message, unsigned int style, bool *ret)
{
    QString strTitle = tr("Kawkaw"); // default title
...
void KawkawGUI::changeEvent(QEvent *e)
{
...
void KawkawGUI::closeEvent(QCloseEvent *event)
{
#ifndef Q_OS_MAC // Ignored on Mac
    if(clientModel && clientModel->getOptionsModel())
    {
...
void KawkawGUI::showEvent(QShowEvent *event)
{
...
#ifdef ENABLE_WALLET
void KawkawGUI::incomingTransaction(const QString& date, int unit, const CAmount& amount, const QString& type, const QString& address, const QString& label, const QString& assetName)
{
    // On new transaction, make an info balloon
    QString msg = tr("Date: %1\n").arg(date);
    if (assetName == "KAW")
        msg += tr("Amount: %1\n").arg(KawkawUnits::formatWithUnit(unit, amount, true));
    else
        msg += tr("Amount: %1\n").arg(KawkawUnits::formatWithCustomName(assetName, amount, MAX_ASSET_UNITS, true));
...
void KawkawGUI::dragEnterEvent(QDragEnterEvent *event)
{
    // Accept only URIs
...
void KawkawGUI::dropEvent(QDropEvent *event)
{
    if(event->mimeData()->hasUrls())
    {
...
bool KawkawGUI::eventFilter(QObject *object, QEvent *event)
{
...
#ifdef ENABLE_WALLET
bool KawkawGUI::handlePaymentRequest(const SendCoinsRecipient& recipient)
{
    // URI has to be valid
...
void KawkawGUI::setHDStatus(int hdEnabled)
{
    QString icon = "";
...
void KawkawGUI::setEncryptionStatus(int status)
{
    switch(status)
    {
...
void KawkawGUI::showNormalIfMinimized(bool fToggleHidden)
{
    if(!clientModel)
        return;
...
void KawkawGUI::toggleHidden()
{
    showNormalIfMinimized(true);
}

void KawkawGUI::detectShutdown()
{
...
void KawkawGUI::showProgress(const QString &title, int nProgress)
{
    if (nProgress == 0)
    {
...
void KawkawGUI::setTrayIconVisible(bool fHideTrayIcon)
{
    if (trayIcon)
    {
...
void KawkawGUI::showModalOverlay()
{
    if (modalOverlay && (progressBar->isVisible() || modalOverlay->isLayerVisible()))
        modalOverlay->toggleVisibility();
}

static bool ThreadSafeMessageBox(KawkawGUI *gui, const std::string& message, const std::string& caption, unsigned int style)
{
...
static bool ThreadSafeMnemonic(KawkawGUI *gui, unsigned int style)
{
    bool modal = (style & CClientUIInterface::MODAL);
    // The SECURE flag has no effect in the Qt GUI.
    // bool secure = (style & CClientUIInterface::SECURE);
...
void KawkawGUI::subscribeToCoreSignals()
{
...
void KawkawGUI::unsubscribeFromCoreSignals()
{
...
void KawkawGUI::toggleNetworkActive()
{
    if (clientModel) {
...
void KawkawGUI::handleRestart(QStringList args)
{
    if (!ShutdownRequested())
    ...
    menu(0)
{
    ...
    QList<KawkawUnits::Unit> units = KawkawUnits::availableUnits();
    int max_width = 0;
    const QFontMetrics fm(font());
    for (const KawkawUnits::Unit unit : units)
    {
    	#ifndef QTversionPreFiveEleven
        	max_width = qMax(max_width, fm.horizontalAdvance(KawkawUnits::name(unit)));
        #else
        	max_width = qMax(max_width, fm.width(KawkawUnits::name(unit)));
        #endif
    }
...
{
    menu = new QMenu(this);
    for (KawkawUnits::Unit u : KawkawUnits::availableUnits())
    {
        QAction *menuAction = new QAction(QString(KawkawUnits::name(u)), this);
...
{
    setText(KawkawUnits::name(newUnits));
}
...
void KawkawGUI::getPriceInfo()
{
    request->setUrl(QUrl("https://api.binance.com/api/v1/ticker/price?symbol=KAWBTC"));
    networkManager->get(*request);
}

#ifdef ENABLE_WALLET
void KawkawGUI::mnemonic()
{
...
void KawkawGUI::getLatestVersion()
{
    versionRequest->setUrl(QUrl("https://api.github.com/repos/KawkawChain/Kawkaw/releases"));
