...
#include "kawkawaddressvalidator.h"
...
void setupAddressWidget(QValidatedLineEdit *widget, QWidget *parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a kawkaw address (e.g. %1)").arg(
        QString::fromStdString(DummyAddress(GetParams()))));
#endif
    widget->setValidator(new KawkawAddressEntryValidator(parent));
    widget->setCheckValidator(new KawkawAddressCheckValidator(parent));
}
...
bool parseKawkawURI(const QUrl &uri, SendCoinsRecipient *out)
{
    // return if URI is not valid or is no kawkaw: URI
    if(!uri.isValid() || uri.scheme() != QString("kawkaw"))
        return false;
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++)
    {
        ...
        else if (i->first == "amount")
        {
            if(!i->second.isEmpty())
            {
                if(!KawkawUnits::parse(KawkawUnits::KAW, i->second, &rv.amount))
                {
                    return false;
                }
            }
...
bool parseKawkawURI(QString uri, SendCoinsRecipient *out)
{
    // Convert kawkaw:// to kawkaw:
    //
    //    Cannot handle this later, because kawkaw:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("kawkaw://", Qt::CaseInsensitive))
    {
        uri.replace(0, 10, "kawkaw:");
    }
    QUrl uriInstance(uri);
    return parseKawkawURI(uriInstance, out);
}

QString formatKawkawURI(const SendCoinsRecipient &info)
{
    QString ret = QString("kawkaw:%1").arg(info.address);
    int paramCount = 0;

    if (info.amount)
    {
        ret += QString("?amount=%1").arg(KawkawUnits::format(KawkawUnits::KAW, info.amount, false, KawkawUnits::separatorNever));
        paramCount++;
    }
...
bool openKawkawConf()
{
    fs::path pathConfig = GetConfigFile(gArgs.GetArg("-conf", KAWKAW_CONF_FILENAME));

    /* Open kawkaw.conf with the associated application */
...
fs::path static StartupShortcutPath()
{
    ...
    if (chain == CBaseChainParams::MAIN)
        return GetSpecialFolderPath(CSIDL_STARTUP) / "Kawkaw.lnk";
    if (chain == CBaseChainParams::TESTNET) // Remove this special case when CBaseChainParams::TESTNET = "testnet4"
        return GetSpecialFolderPath(CSIDL_STARTUP) / "Kawkaw (testnet).lnk";
    return GetSpecialFolderPath(CSIDL_STARTUP) / strprintf("Kawkaw (%s).lnk", chain);
}

bool GetStartOnSystemStartup()
{
    // check for Kawkaw*.lnk
...
fs::path static GetAutostartFilePath()
{
    std::string chain = ChainNameFromCommandLine();
    if (chain == CBaseChainParams::MAIN)
        return GetAutostartDir() / "kawkaw.desktop";
    return GetAutostartDir() / strprintf("kawkaw-%s.lnk", chain);
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        std::string chain = ChainNameFromCommandLine();
        // Write a kawkaw.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        if (chain == CBaseChainParams::MAIN)
            optionFile << "Name=Kawkaw\n";
        else
            optionFile << strprintf("Name=Kawkaw (%s)\n", chain);
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    ...
    // loop through the list of startup items and try to find the kawkaw app
...
bool GetStartOnSystemStartup()
{
    CFURLRef kawkawAppUrl = CFBundleCopyBundleURL(CFBundleGetMainBundle());
    if (kawkawAppUrl == nullptr) {
        return false;
    }
    
    LSSharedFileListRef loginItems = LSSharedFileListCreate(nullptr, kLSSharedFileListSessionLoginItems, nullptr);
    LSSharedFileListItemRef foundItem = findStartupItemInList(loginItems, kawkawAppUrl);

    CFRelease(kawkawAppUrl);
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if(fAutoStart && !foundItem) {
        // add kawkaw app to startup item list
        LSSharedFileListInsertItemURL(loginItems, kLSSharedFileListItemBeforeFirst, nullptr, nullptr, kawkawAppUrl, nullptr, nullptr);
    }
